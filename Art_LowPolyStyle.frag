...
// by Tom'2016
// Modified by Ruofei to fix the boundaries
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec4 tri = texture2D(iChannel0, uv);
    ...
    vec3 c0 = texture2D(iChannel1, p0 / iChannelResolution[1].xy ).xyz;
    vec3 c1 = texture2D(iChannel1, p1 / iChannelResolution[1].xy ).xyz;
    vec3 c2 = texture2D(iChannel1, p2 / iChannelResolution[1].xy ).xyz;
    vec3 c3 =texture2D(iChannel1, p3 / iChannelResolution[1].xy ).xyz;
#if GAMMA_CORRECTION
...
#endif
    if (iMouse.w > 0.0) {
    	fragColor = vec4(texture2D(iChannel3, uv).xyz, 1.0);    
    }
}