...
vec4 FindTriangles (in vec2 fragCoord )
{
    ...
    float offset0 = texture2D( iChannel1, baseCoord ).x;
    float offset1 = texture2D( iChannel1, baseCoord + one.zy ).x;
    float offset2 = texture2D( iChannel1, baseCoord + one.xy ).x;
    float offset3 = texture2D( iChannel1, baseCoord + one.xz ).x;
    // Keep CW order
    // 1 2
    // 0 3
...
vec4 StepTriJFA (in vec2 fragCoord, in float level)
{
    ...
    for (int y = -1; y <= 1; ++y) {
        for (int x = -1; x <= 1; ++x) {
            ...
            vec4 tri = texture2D( iChannel0, sampleCoord / iChannelResolution[0].xy);
            if (tri.x == 0.) continue;
